...
Conv1DLayer::Conv1DLayer(Tensor t, float* b, int s) {
    this->weights = t;
    this->biases = b;
    this->stepSize = s;
}
...
void Conv1DLayer::output(Matrix iMatrix, Matrix* oMatrix) {
    int outputSamples = (iMatrix.nSamples - this->weights.nSamples) / stepSize + 1;
        ...
        for (int j = 0; j < outputSamples; j++) {
            ...
            for (int k = 0; k < this->weights.nSamples; k++) {
                ...
                for (int m = 0; m < iMatrix.nAxes; m++) {
                    total += iMatrix.getValue(k + stepSize * j, m) * this->weights.getValue(k, m, i);
                }
            }
...
int main() {
    ...
    Conv1DLayer layer1 = Conv1DLayer(conv1Tensor, conv1Biases, CONV1STEPSIZE);
    ...
    Conv1DLayer layer2 = Conv1DLayer(conv2Tensor, conv2Biases, CONV2STEPSIZE);
